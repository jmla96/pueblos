...
export class CanvasPlatformer2D {
	...
	constructor(canvas: HTMLCanvasElement, public home?: any) {
		this.canvas = canvas;
		...
		window.addEventListener('resize', () => this.setupHighResolutionCanvas());
	}

	private setupHighResolutionCanvas(): void {
		// Obtener el DPI del dispositivo
		const devicePixelRatio = window.devicePixelRatio || 1;
		
		// Obtener el tamaño de visualización del canvas
		...
		// Escalar el contexto para compensar el DPI
		this.ctx.scale(devicePixelRatio, devicePixelRatio);
		
		// Configurar el tamaño de visualización en CSS
		...
		console.log(`Canvas configurado: ${this.canvas.width}x${this.canvas.height} (${devicePixelRatio}x DPI)`);
	}
	...
	private generateInitialWorld(): void {
		...
		// Create initial obstacles
		   // No generar obstáculos peligrosos cerca del inicio del jugador
		   for (let i = 0; i < 10; i++) {
			   let x = 300 + i * 200 + Math.random() * 100;
			   // Si el obstáculo está demasiado cerca del jugador, solo permite monedas
			   if (x < 400) {
				   this.createObstacle(x, "coin");
			   } else {
				   const type = Math.random() < 0.6 ? "coin" : "spike";
				   this.createObstacle(x, type);
			   }
		   }
		this.lastObstacleX = 600; // Asegura que los obstáculos generados estén lejos del inicio
	}
	...
	private setupControls(): void {
		...
		window.addEventListener('keydown', (e) => {
			if (!this.keys[e.code]) { // Prevent key repeat
	...
	private updatePlayer(): void {
		// Horizontal movement - automatic forward + optional manual control
		this.player.velocityX = this.gameState.speed; // Always moving forward
		
		// Allow player to move left/right a bit for dodging
		if (this.keys['ArrowLeft'] || this.keys['KeyA']) {
			this.player.velocityX *= 0.5; // Slow down
		} else if (this.keys['ArrowRight'] || this.keys['KeyD']) {
			this.player.velocityX *= 1.5; // Speed up
		}
		...
		// Set isGrounded based on ground or platform collision
		this.player.isGrounded = groundCollision || platformCollision;
	...
	private updateCamera(): void {
		// Follow player with smooth camera (player at 10% of canvas width)
		const targetCameraX = this.player.x - this.canvas.width * 0.1 + this.player.width / 2;
	...
	private drawUI(): void {
		   ...
		   this.ctx.beginPath();
		   this.ctx.moveTo(15, 15 + 10);
		   this.ctx.arcTo(15, 15, 15 + 10, 15, 10);
		   this.ctx.lineTo(15 + panelWidth - 10, 15);
		   this.ctx.arcTo(15 + panelWidth, 15, 15 + panelWidth, 15 + 10, 10);
		   this.ctx.lineTo(15 + panelWidth, 15 + panelHeight - 10);
		   this.ctx.arcTo(15 + panelWidth, 15 + panelHeight, 15 + panelWidth - 10, 15 + panelHeight, 10);
		   this.ctx.lineTo(15 + 10, 15 + panelHeight);
		   this.ctx.arcTo(15, 15 + panelHeight, 15, 15 + panelHeight - 10, 10);
		   this.ctx.closePath();
		   this.ctx.fill();
		   this.ctx.stroke();
		   ...
		   this.ctx.fillText(`Score: ${this.gameState.score}`, 30, 40);
		   this.ctx.fillText(`Distance: ${this.gameState.distance}m`, 30, 65);

		   // --- Lives as hearts ---
		   this.ctx.fillText('Lives: ', 30, 90);
		   for (let i = 0; i < this.gameState.lives; i++) {
			   ...
			   this.ctx.bezierCurveTo(x + 10, y, x, y, x, y + 8);
			   this.ctx.bezierCurveTo(x, y + 15, x + 10, y + 18, x + 10, y + 23);
			   this.ctx.bezierCurveTo(x + 10, y + 18, x + 20, y + 15, x + 20, y + 8);
			   this.ctx.bezierCurveTo(x + 20, y, x + 10, y, x + 10, y + 8);
			   this.ctx.closePath();
			   this.ctx.fillStyle = '#e53935';
			   this.ctx.shadowColor = '#b71c1c';
		   ...
		   // --- Speed indicator ---
		   this.ctx.fillStyle = '#222';
		   this.ctx.fillText(`Speed: ${this.gameState.speed.toFixed(1)}`, 30, 115);

		   // --- Controls hint (desktop only) ---
		   if (!this.gameState.isRunning && !this.gameState.isGameOver && window.innerWidth > 768) {
			   this.ctx.fillStyle = '#444';
			   this.ctx.font = '16px Arial';
			   this.ctx.fillText('SPACE/UP to jump, A/D for speed', 30, this.canvas.height - 20);
		   }
	}
	...
	public jump(): void {
		if (this.player.isGrounded && this.gameState.isRunning) {
			this.player.velocityY = this.jumpForce;
			this.player.isGrounded = false;
			console.log('Jump executed! VelocityY:', this.player.velocityY);
		} else {
			console.log('Jump blocked - isGrounded:', this.player.isGrounded, 'isRunning:', this.gameState.isRunning);
		}
	}

	public setLeftPressed(pressed: boolean): void {
		this.keys['ArrowLeft'] = pressed;
	}

	public setRightPressed(pressed: boolean): void {
	...
	public dispose(): void {
		...
		window.removeEventListener('resize', () => this.setupHighResolutionCanvas());
...
export { CanvasPlatformer2D as BabylonSceneFour };